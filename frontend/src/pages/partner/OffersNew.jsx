// src/pages/partner/OffersNew.jsx
...
export default function OffersNew() {
  ...
  // Stations ophalen (optioneel; als er geen endpoint is, blijft de selector leeg)
  useEffect(() => {
    (async () => {
      try {
        const data = await apiJson("/api/partner/stations");
        const rows = Array.isArray(data) ? data : (data?.stations || data?.rows || []);
        setStations(rows || []);
      } catch {
        setStations([]);
      }
  ...
  async function handleSubmit(e) {
    ...
    try {
      const payload = {
        ...
        starts_at: form.starts_at || null, // laat backend TZ verwerken
      ...
      setOkMsg("Aanbieding opgeslagen ✅");
  ...
  return (
    <PartnerLayout title="Nieuwe aanbieding">
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "8px 12px 40px" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, marginBottom: 12 }}>
          <h1 style={{ fontSize: 22, margin: 0 }}>Nieuwe aanbieding</h1>
